...
ll search( ll start, ll end, pair<ll,ll> p ) {
...
/*	  for( ll j = i-1; j >= 0; j-- )
	...
	*/ ll j;
	   if( i-1 >= 0 && v[i-1].F == v[i].F )
	   	 j = i-1;
	   else
	    j = search( 0, i-1, mp( v[i].F.F-1, m-1 ) );
//	   cout << " I: " << i << " J : " << j << endl;
	   if( j >= 0 && v[i].F.F >= v[j].F.F ) {
	   ...
	   if( i-1 >= 0 && v[i].F == v[i-1].F )
	   	dp[i] += dp[i-1];
	   j = search( 0, i-1, mp( v[i].S.F-1, m-1 ) );
//	   cout << dp[i] << ":: I2: " << i << " J : " << j << endl;

	   if( j >= 0 && ( v[i].S.F > v[j].F.F || v[i].S == v[j].F ) ) {
		  dp[i] += dp[j]*modpow(m,v[i].S.F-v[j].F.F-1);
	   }
//	   cout << dp[i] << ":::: " << endl;
		
		dp[i] += modpow( m, v[i].S.F-1 ); 
		dp[i] %= MOD;
