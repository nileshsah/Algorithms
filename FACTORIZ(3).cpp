...
int main() {
    ...
    while(t--) {
      ...
      // ctick++;
       ...
       if( str.size() >= 20 ) {
         flag = 1;
         /*for( int k = divi.size()-1; k >= 0; k-- )
               if( prime[i]%( divi[k].first ) != divi[k].second )
                 { flag = 0; break; }
           */        
         ...
         if( flag )
          { solint.pb(prime[i]);  str = temp;  }
         if( !flag )
          divi.pb( make_pair( prime[i], rem ) );
         cout << prime[i] << " " << temp << " " << rem << endl;
          if( tock > 130 )
